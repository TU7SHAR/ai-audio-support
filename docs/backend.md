...
```
[system prompt]  (+ language instruction if a language is chosen)
[history turns]  (user/assistant, in order — currently ALL of them)
...
- **Stateless:** no server-side sessions or memory; see
  [`conversation-memory.md`](./conversation-memory.md).
- **History is trusted blindly** and replayed in full → context-window and
  latency risk on a CPU box, plus a forged-history vector.
